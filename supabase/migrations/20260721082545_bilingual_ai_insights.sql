-- RepoPulse bilingual AI insights.

alter table public.repositories
  add column if not exists ai_category_zh text,
  add column if not exists ai_summary_zh text,
  add column if not exists ai_audience_zh text,
  add column if not exists ai_reason_zh text,
  add column if not exists ai_signals_zh text[] not null default '{}';

drop function if exists public.get_repository_rankings(integer, integer);

create function public.get_repository_rankings(
...
as $$
  ...
  ranked as (
    select
      ...
      repository.language,
      repository.topics,
      repository.ai_category,
      repository.ai_category_zh,
      repository.ai_summary,
      repository.ai_summary_zh,
      repository.ai_audience,
      repository.ai_audience_zh,
      repository.ai_reason,
      repository.ai_reason_zh,
      repository.ai_signals,
      repository.ai_signals_zh,
      repository.ai_model,
      repository.ai_enriched_at,
      latest.stars_count,
      latest.forks_count,
  ...
  select
    ...
    ranked.topics,
    ranked.ai_category,
    ranked.ai_category_zh,
    ranked.ai_summary,
    ranked.ai_summary_zh,
    ranked.ai_audience,
    ranked.ai_audience_zh,
    ranked.ai_reason,
    ranked.ai_reason_zh,
    ranked.ai_signals,
    ranked.ai_signals_zh,
    ranked.ai_model,
    ranked.ai_enriched_at,
    ranked.stars_count,
    ranked.forks_count,
    ranked.star_gain,
    ranked.fork_gain,
